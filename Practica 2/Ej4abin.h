...
template <typename T> class ABIN_H
{
public:
	static const T NODO_NULO;
	Abin(unsigned tam);
...
private:
	T *elem;
	unsigned tam_,altMax_;
}

/* Definición del nodo nulo */
template <typename T>
const typename Abin<T>::nodo Abin<T>::NODO_NULO(T());

/*--------------------------------------------------------*/
/* Métodos públicos */
/*--------------------------------------------------------*/
template <typename T>
inline Abin<T>::Abin(unsigned tam) : altMax_(altMax), elem(new T[pow(5,2)-1])
{
	tam = pow(5,2)-1;
}
...
{
	assert(elem[(tam_-1)/2] == NODO_NULO); // Árbol vacío
	elem[(tam_-1)/2] = e;
}

template <typename T>
inline void Abin<T>::insertarHijoIzqdo(unsigned pos, const T& e)
{
	assert(elem[pos] != NODO_NULO);
	unsigned i = 
...
template <typename T>
inline void Abin<T>::insertarHijoDrcho(unsigned pos, const T& e)
{
	assert(n != NODO_NULO);
