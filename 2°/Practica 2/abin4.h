...
template <typename T>
inline Abin4<T>::Abin4(int n) : r(NODO_NULO), nNodos(n) {
    nodos = new std::vector<T>(n);
}
...
template <typename T>
inline bool Abin4<T>::arbolVacio() const
{
    return (r == NODO_NULO);
}
...
template <typename T>
inline
    typename Abin4<T>::nodo
    Abin4<T>::padre(Abin4<T>::nodo n) const
{
    assert(n != NODO_NULO);
    return n->padre;
}
...
inline
    typename Abin4<T>::nodo
    Abin4<T>::hijoIzqdo(Abin4<T>::nodo n) const
{
    assert(n != NODO_NULO);
    return n->hizq;
}
...
inline
    typename Abin4<T>::nodo
    Abin4<T>::hijoDrcho(Abin4<T>::nodo n) const
{
    assert(n != NODO_NULO);
    return n->hder;
}
